...
object TypeRep {
  ...
  /**
   * Creates an anonymous constant with the given value.
   * Assumes the value is not symbolic and is of the
   * type given.
  ...
  /**
   * Represents a data type in MXDR. The underlying type of the data is communicated by
   * the name of the inheriting class. Stores additional data about the type.
   */
  abstract class Rep(
                      /**
    ...
    /**
     * True if this type has an explicitly declared subtype.
     * If true, baseTypeName is present.
     */
    val isSubtype: Boolean = baseTypeName.nonEmpty

    /**
     * The user defined type name of this type.
     * An array of "double" (where double is an XDR type)
     * is an array of elements without a type name.
     */
    val typeName: Option[String] = None

    /**
     * Anonymous types might occur as array components
     * in a struct.
     */
...
//    def maxSize(): Int
    ...
    // Check that we didn't accidentally set the type value
    // to an Option[String]. (Scala doesn't do a type check here,
   ...
   * @param baseTypeName In cases where a "Boolean" is required in the MXDR, the typeName is just
   *                 "Boolean". If a typedef defines a new symbol for a Boolean, then this
   *                 equals that symbol.
   */
  ...
  sealed trait Ranged {
    ...
    /**
     * Defined by the implementing type for cases when no
     * custom range is imposed by the user.
     */
    def implicitLow(): Number
    /**
     * Defined by implementing types for cases when no
     * custom range is imposed by the user.
     */
    ...
    /**
     * True if the range was specified explicitly in the MXDR.
     * If false, the range is implicit to the type.
     */
    val isExplicit: Boolean = explicitRange.nonEmpty

    /**
     * The range of the type. If there is an explicit range, that range.
     * Otherwise, the default implicit range for the type.
     *
     * Even if the explicit/implicit range doesn't have typed
     * endpoints, this value will by inferring them.
     */
  ...
  class DoubleRep(override val typeName: Option[String],
                  override val explicitRange: Option[RangeConstraint],
                  override val baseTypeName: Option[String] = None,
                 ) extends ContinuousRep(baseTypeName) {
   ...
   * @param maxBytes The maximum number of bytes this opaque type may hold.
   *                 If omitted, a default value specific to Merc is used.
   * @param typeName The name of the type.
   */
  class VariableOpaqueRep(override val typeName: Option[String],
                          val maxBytes: ConstRep[IntegralRep] = anonymousConstant(MAX_OPAQUE_LENGTH.toString)
                         ) extends OpaqueRep
   ...
   */
  class FixedOpaqueRep(override val typeName: Option[String],
                       val bytes: ConstRep[IntegralRep],
                      ) extends OpaqueRep

  class QuadRep(override val typeName: Option[String],
                override val explicitRange: Option[RangeConstraint],
                override val baseTypeName: Option[String] = None
               ) extends IntegralRep(baseTypeName) {
   ...
   *                  Note that in XDR, one byte is one ASCII character.
   *                  Strings are prefixed by a 32-bit uint for the length.
   *                  If unspecified, defaults to the maximum length
   *                  that can be stored in a 32-bit int. If specified,
   *                  must be lower than the max string length.
   * Note that there is no Fixed-length string type. All strings
   * are variable length.
   */
  class StringRep(override val typeName: Option[String],
   ...
   * A time type.
   */
  class TimeRep(override val typeName: Option[String],
                override val baseTypeName: Option[String] = None
               ) extends Rep(baseTypeName)
  class TimeSpanRep(override val typeName: Option[String],
                    override val baseTypeName: Option[String] = None
                   ) extends Rep(baseTypeName)
  class UHyperRep(override val typeName: Option[String],
                  override val explicitRange: Option[RangeConstraint] = None,
                  override val baseTypeName: Option[String] = None
                 ) extends IntegralRep(baseTypeName) {
  ...
  class UIntRep(override val typeName: Option[String],
                override val explicitRange: Option[RangeConstraint] = None,
                override val baseTypeName: Option[String] = None
               ) extends IntegralRep(baseTypeName) {
  ...
  /**
   * Represents a symbol with an associated value.
   * Optionally stores the type of the value.
   */
  class ConstRep[+T <: Rep](
                            /**
                             * The name of the symbol, or null if the constant is a literal.
                             */
                            val name: Option[String],

                            /**
                             * The value of the symbol. May be the name of another constant, or a value.
                             * Ex)
                             * Type'Last
                             * 10
                             * 3.14
                            ...
                            /**
                            * True if the value is a symbol, false if it can be converted strait
                            * to its value via ".toInt"
                            */
                            val isSymbolic: Boolean,

                            /**
                             * The string in the MXDR of the type.
                             */
                            val typeName: Option[String],
                            /**
                             * The type of the constant, if the type was specified.
                             */
                            val typeRep: Option[T]
  ...
  abstract class ArrayRep extends Rep(None) with StructuredRep {
     ...
     * Must be of a named type.
     */
    val elementType: Rep

    /**
     * Creates a new array but with the given
     * element type.
     */
   ...
   * @param size A constant representing the length
   *             of the array
   * @param elementType The element type of the array.
   */
  class FixedArrayRep(override val typeName: Option[String],
                      val size: ConstRep[IntegralRep],
   ...
   * @param elementType The element type of the array.
   * @param maxSize The maximum length of the array. If no value is
   *                specified, defaults to a maximum array length.
   */
  class VariableArrayRep(override val typeName: Option[String],
                         override val elementType: Rep,
                         val maxSize: ConstRep[IntegralRep] = anonymousConstant(MAX_ARRAY_LENGTH.toString)
                        ) extends ArrayRep {
   ...
   * @param rawTypeName The string that appeared in the MXDR
   *                    as the type name. May be a fundamental
   *                    XDR type, or an identifier representing
   *                    a user defined type.
   ...
   * @param value Optional value parameter for enum values. May be a
   *              symbol or just a number.
   */
   ...
   * @param values The values the enum can take on. All value names
   *               are unique in the enum. They either all have values
   *               or all don't. If they have values, all the values are
   *               unique. Has at least one value.
   */
  class EnumRep   (override val typeName: Option[String],
                   val values: List[EnumValue]
                  ) extends Rep(None) with StructuredRep {
    repOk()
   ...
   * @param components The components of the struct. All component names
   *                   are unique. If one of the components is type VOID,
   *                   it is the only component.
   */
  class StructRep (override val typeName: Option[String],
                   val components: List[StructComponent]
                  ) extends Rep(None) with StructuredRep {
    /**
     * Gets the component for the given name.
     * Assumes a component with the given name exists.
     */
    def getComponent(name: String): StructComponent = components.find(_.name == name).get

    /**
     * True if the message has one component and
     * that component has type void.
     */
    val isVoid: Boolean = components.size == 1 && components.head.typeRep == VoidRep

    /**
     * Creates a new StructRep but with the indicated component's
     * type replaced by the given type. If the indicated component
     * isn't found, throws. If multiple components match
     * the indicator, replace all of them.
   ...
   *                   are unique. If one of the components is type VOID,
   *                   it is the only component.
   * @param direction Whether the message is sent or received by
   *                  the given module.
   * @param invariants List of user specified invariants for the message.
   *                   Strings take the form of expressions.
   */
  class MStructRep(override val typeName: Option[String],
                  ...
                  ) extends StructRep(typeName, components) {
  ...
  @unused
  class UnionRep  (override val typeName: Option[String],
                   @unused val name: String,
                   @unused val components: StructComponent,
                   override val baseTypeName: Option[String] = None
                  ) extends Rep(baseTypeName) with StructuredRep
   ...
   */
  class RangeConstraint(val lowerBound: ConstRep[NumericRep],
                        val upperBound: ConstRep[NumericRep])
